...
class ParsingFileProcessorTest extends TestCase
{
    public function testProcessing()
    {
            ...
            public function parse(string $content, ClassDataHandler $handler): void
            {
                if ('content1' === $content) {
                    $handler->handle(
                        new FileInfo(
                            'Namespace1',
                            (new StringList())->add('Namespace3\Class1')->add('Namespace4\Class1')
                        )
                    );
                }
                if ('content2' === $content) {
        ...
        $this->assertEquals(
            [
                'Namespace3' => new Dependency($arr['Namespace3'], 1),
                'Namespace4' => new Dependency($arr['Namespace4'], 1)
            ],
            iterator_to_array($arr['Namespace1']->getDependencies())
        );
        $this->assertEquals(
            ['\\' => new Dependency($arr['\\'], 1)],
